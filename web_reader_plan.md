...
- [ ] 实现配置读取、默认 `:8848`、workspace 启动校验。
- [ ] 实现 `hash-password` 子命令。
- [ ] 实现 admin 登录、session 查询、退出。
- [ ] 实现 session 过期清理和登录限流。
- [ ] 实现认证 middleware，保护全部 `/api/fs/*`。
- [ ] 实现安全响应头、请求 ID、结构化日志和 panic recovery。
- [ ] 实现 HTTP server timeout 和优雅关闭。
- [ ] 完成登录页及全局 `401` 处理。
...
- [ ] 实现唯一的安全路径解析函数。
- [ ] 覆盖 `../`、绝对路径、编码路径、符号链接逃逸等测试。
- [ ] 实现懒加载目录 API 和自然排序。
- [ ] 实现文件元信息及 `previewKind` 判断。
- [ ] 实现有限大小文本读取和编码错误处理。
- [ ] 实现支持 Range/缓存协商的 raw 文件流。
- [ ] 实现主动内容下载策略和 raw CSP。
- [ ] 统一 JSON 错误格式。
